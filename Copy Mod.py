...
class MainWindow(wx.Frame):
    def __init__(self, parent, title):
       ...
       # wx.StaticText(panel, -1, "Copy text files modified in last 24hrs", (150,10))
        ...
        wx.Button(panel, 3, "Copy Files", (160,100))
        self.Bind(wx.EVT_BUTTON, self.copyFiles, id=3)

        # Create the file check button
        wx.Button(panel, 4, "File Check", (250, 100))
        self.Bind(wx.EVT_BUTTON, self.fileCheck, id=4)
    ...
    def fileCheck(self, event):
        ...
        c.execute("INSERT INTO timeStamp (ID, unix, datestamp) VALUES(?,?,?)",
                  (idforddb, time.time(), date))
        conn.commit()
